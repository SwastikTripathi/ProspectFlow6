...
export default function BlogPostPage() {
  ...
  const handleScroll = useCallback(() => {
    ...
    // Iterate from top to bottom, find the LAST heading that is AT or ABOVE the activation point
    for (let i = 0; i < headingElementsRef.current.length; i++) {
      ...
      if (rect.top <= activationPoint) {
        ...
        // If this heading is below the line, the previous one (if any) was the last one above the line
        break; 
      }
    }
    
    // If no heading is above or at the activation line (e.g., scrolled to the very top)
    // and there are headings, default to the first one.
  ...
  }, [activeHeadingId]); // Removed NAVBAR_HEIGHT_OFFSET as it's a constant defined outside
  ...
  useEffect(() => {
    ...
    if (mainContentRef.current) {
      ...
      const newTocItems = headings.map((heading, index) => {
        ...
        // Level is always 1 since we are querying H1s
        let id = heading.id;
        // ID should already be generated by the mdxComponent for h1
        if (!id) { // Fallback just in case
  ...
  const mdxComponents = {
    h1: (props: React.HTMLAttributes<HTMLHeadingElement>) => {
      ...
      let id = props.id;
      if (!id) {
        const childrenText = React.Children.toArray(children).filter(child => typeof child === 'string').join('');
        id = childrenText.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '') || `heading-${Math.random().toString(36).substring(7)}`;
      }
      return (
        <h1
          id={id}
          ...
          className={cn("text-xl font-semibold tracking-tight", propClassName)} 
        >
          {children}
        </h1>
      );
    ...
    h2: (props: React.HTMLAttributes<HTMLHeadingElement>) => {
      const { children, className: propClassName, ...rest } = props;
      return <h2 {...rest} className={cn("text-lg font-semibold tracking-tight", propClassName)}>{children}</h2>;
    },
    h3: (props: React.HTMLAttributes<HTMLHeadingElement>) => {
      const { children, className: propClassName, ...rest } = props;
      return <h3 {...rest} className={cn("text-base font-semibold tracking-tight", propClassName)}>{children}</h3>;
    },
    h4: (props: React.HTMLAttributes<HTMLHeadingElement>) => {
      const { children, className: propClassName, ...rest } = props;
      return <h4 {...rest} className={cn("text-sm font-semibold tracking-tight", propClassName)}>{children}</h4>;
    },
  };
      ...
      <main className="flex-1 py-12 md:py-16">
        <div className="container mx-auto px-[5vw] md:px-[8vw] lg:px-[10vw] max-w-screen-xl">
           ...
           <div className="lg:grid lg:grid-cols-12 lg:gap-x-12 xl:gap-x-16 mb-8">
            ...
            <div className="lg:col-span-4 hidden lg:block"></div> {/* Empty column to match ratio for image */}
          </div>
